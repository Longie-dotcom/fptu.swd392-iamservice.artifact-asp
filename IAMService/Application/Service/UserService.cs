...
namespace Application.Service
{
    public class UserService : IUserService
    {
            ...
            string role)
        {
            ...
            // Publish message
            await userDeletePublisher.PublishAsync(new SWD392.MessageBroker.UserDeleteDTO()
            {
                UserID = user.UserID,
            });
        }
        ...
        public async Task UserSyncUpdating(SWD392.MessageBroker.UserUpdateDTO dto)
        {
            var user = await unitOfWork
                .GetRepository<IUserRepository>()
        ...
        public async Task<Guid> CreateUserGrpcAsync(
            CreateUserRequest request)
        {
            if (string.IsNullOrWhiteSpace(request.Role))
                throw new RoleNotFound("User must have at least one role.");

            // Validate role existence
            var role = await unitOfWork
                .GetRepository<IRoleRepository>()
                .GetByCodeAsync(request.Role);

            if (role == null)
                throw new RoleNotFound(
                    $"Role code '{request.Role}' does not exist.");

            ValidateRoleModifying(request.Role, role);
            ...
            if (existingUser != null)
            {
                ...
                bool hasRole = existingUser.UserRoles
                    .Any(ur => ur.RoleID == role.RoleID && ur.IsActive);

                if (hasRole)
                    throw new UserAlreadyExists(
                        $"User with email '{request.Email}' already has role '{request.Role}'.");

                // CASE 2: Add new role
                existingUser.AddRole(role.RoleID);

                userRepo.Update(existingUser.UserID, existingUser);

                await unitOfWork.CommitAsync(existingUser.UserID.ToString());
